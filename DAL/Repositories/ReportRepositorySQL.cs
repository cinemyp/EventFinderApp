...
namespace DAL.Repositories
{
    public class ReportRepositorySQL : IReportRepository
    {
        ...
        {
            List<DAL.Session> sessions = db.User.Find(userId).Session.Where(i => i.Date.DayOfYear < DateTime.Now.DayOfYear && i.Date.Month == month).ToList();
            ...
            List<Session> events = db.User.Find(userId).Session
                .Where(i => i.Date.DayOfYear < DateTime.Now.DayOfYear)
                .ToList();
